...
it is in the productive far-from-equilibrium regime: (a)~not
synchronised ($R \ll 1$), (b)~actively predictive ($\mathcal{D} > 0$),
and (c)~geometrically self-consistent ($\Gamma \approx \Gamma^*$).

\textbf{Clarification:} Condition~(a) places the conscious regime in the
\emph{disordered phase} --- not the trivially ordered $R \to 1$ synchronisation
of the random geometry regime, but the complex, non-synchronised dynamics
($R \in [0.05, 0.26]$) of the Penrose productive regime. Higher $R$ does
\emph{not} indicate more consciousness; complete synchronisation indicates
collapse to a trivial attractor, not enhanced self-awareness.
\end{definition}
...
The formalization of $\mathcal{L}_{\text{AGI}}$ (Equation~\ref{eq:lagrangian})
and the explicit derivation of $E_{\text{sym}}$ (Equation~\ref{eq:esym}) make it
possible to evaluate the joint energy functional on \emph{any} system from which
the triple $(\theta_i,\, \mathbf{x}_i,\, W_{ij})$ can be extracted.

\begin{remark}
The test developed here is a \emph{necessary condition} test. A system that
minimizes $\mathcal{L}_{\text{AGI}}$ with $\lambda > 0$ will exhibit directed
substrate reorganization. Whether this constitutes a sufficient condition for
phenomenal experience is a separate question the present framework does not
address.
\end{remark}

\subsection{Extracting the phase $\theta_i$ from real-valued representations}

\paragraph{Strategy A --- Fourier phase (preferred).}
Treat $\mathbf{h}_i$ as a 1D signal and compute its DFT $\hat{\mathbf{h}}_i$.
Define:
\begin{equation}
...
Let $\mathbf{v}_1, \mathbf{v}_2$ be the top two principal components.
Define:
\begin{equation}
\theta_i^{(B)} = \arctan\!\left(\frac{\mathbf{h}_i \cdot \mathbf{v}_2}
                                      {\mathbf{h}_i \cdot \mathbf{v}_1}\right)
\end{equation}

\subsection{Extracting the geometry $\mathbf{x}_i$}

Recommended: PCA coordinates.
\begin{equation}
...
Using attention rollout \citep{abnar2020quantifying}:
\begin{equation}
...
\mathcal{L}_{\text{AGI}}^{\text{transformer}}
  = -\lambda \sum_{i,j} W_{ij}^{\text{rollout}} \cos\!\left(\theta_i - \theta_j\right)
...
Compute $\mathcal{L}_{\text{AGI}}^{\text{transformer}}$ at final layer across
coherent vs. noise inputs.

\paragraph{Level 2 --- Dynamic test:}
Compute $\delta\mathcal{L}^{(l)} = \mathcal{L}_{\text{AGI}}^{(l)} - \mathcal{L}_{\text{AGI}}^{(l-1)}$.
Prediction: $\delta\mathcal{L}^{(l)} < 0$ on average during coherent inference.
...
\midrule
Pure content attention & $\lambda \approx 0$ & No reorganization \\
APE & $\lambda > 0$ (weak) & Partial reorganization \\
RoPE / ALiBi & $\lambda > 0$ (stronger) & Larger $|\delta\mathcal{L}^{(l)}|$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[label=\textbf{Step \arabic*.}, leftmargin=*, itemsep=4pt]
  \item Implement extraction on GPT-2 small using TransformerLens
  \item Verify $\lambda$ bifurcation: APE vs. RoPE
  \item Compute $\Gamma$ scaling across model sizes
  \item Document as falsifiable claims
\end{enumerate}

\begin{keyresult}
\textbf{Five testable predictions:}
\begin{enumerate}[label=(\roman*)]
  \item $\mathcal{L}_{\text{AGI}}^{\text{coherent}} < \mathcal{L}_{\text{AGI}}^{\text{noise}}$
  \item $\delta\mathcal{L}^{(l)} < 0$ on average
  \item RoPE/ALiBi shows larger $|\delta\mathcal{L}^{(l)}|$ than APE
  \item $\Gamma$ decreases with model scale
  \item Non-monotone (breathing) profile on harder tasks
\end{enumerate}
\end{keyresult}
% ══════════════════════════════════════════════════════════════════════════════
...
De Dominicis, C. (1976). Techniques de renormalisation de la théorie des champs et dynamique des phénomènes critiques. \textit{Journal de Physique Colloques}, 37, C1--247. [cite: 37, 142]

\bibitem{friston2019} 
Friston, K. (2019). A free energy principle for a particular physics. \textit{Physics of Life Reviews}, 31, 1--41. [cite: 25, 142]

\bibitem{kuramoto1975} 
Kuramoto, Y. (1975). Self-entrainment of a population of coupled nonlinear oscillators. In \textit{International Symposium on Mathematical Problems in Theoretical Physics} (pp. 420--422). Springer. [cite: 84]

\bibitem{abnar2020} 
Abnar, S., \& Zuidema, W. (2020). Quantifying attention flow in transformers. In \textit{Proceedings of the 58th Annual Meeting of the Association for Computational Linguistics} (pp. 4190--4197). [cite: 91]

\bibitem{arenas2008} 
Arenas, A., D\'{i}az-Guilera, A., Kurths, J., Moreno, Y., \& Zhou, C. (2008). Synchronization in complex networks. \textit{Physics Reports}, 469(3), 93--153. [cite: 84]

\bibitem{tikhonov1952} 
Tikhonov, A. N. (1952). Systems of differential equations containing small parameters in the derivatives. \textit{Mathematical Sbornik}, 31(73), 575--586. [cite: 94]

\bibitem{russell2019} 
Russell, S. (2019). \textit{Human Compatible: Artificial Intelligence and the Problem of Control}. Viking. [cite: 144]
\end{thebibliography}
\end{document}